...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<String>> handleFeignException(FeignException exception) {
        HttpStatus httpStatus = HttpStatus.valueOf(exception.status());
        log.error("FeignException occurred: Status={}, Message={}, ResponseBody={}",
                exception.status(), exception.getMessage(), exception.contentUTF8());
        return new ResponseEntity<>(ApiResponse.error(getGenericMessage(exception.status())), httpStatus);
    }
