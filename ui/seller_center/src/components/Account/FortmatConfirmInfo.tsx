  ...
  return (
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4 bg-white w-full p-5 rounded-xl">
        ...
        <input  {...register('name', { required: true })} type="text" placeholder="Nhập tên cửa hàng" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
        {errors.name?.message && (
      ...
      {option == 0 ?
        (
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-4 bg-white w-full p-5 rounded-xl ">
              <p className="font-bold text-[20px]">Xác nhận thông tin cá nhân</p>
              ...
              <div className="flex gap-1 items-center ">
                ...
                />
                <span className="text-gray-700">Chứng minh nhân dân</span>
                <input
                  type="radio"
                  value={1}
              ...
              <div className="flex flex-col gap-4  ">
                ...
                {typeSubTitile == 0 ? (<>
                  <FormInfo info={info[0]}>
                    <FormInput
                      {...register("identityCardFrontSide", { required: true })}
                      onChange={handleImg}
                    />
                  ...
                  <FormInfo info={info[1]}  >
                    <FormInput  {...register('identityCardBackSide', { required: true })} onChange={handleImg} />
                  </FormInfo>
                </>) :
                  (<>
                    {/* <FormInfo info={info[0]}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input {...register('identityCardName', { required: true })} type="text" placeholder="Nhập giống hệt ảnh đã tải lên" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
                  {errors.identityCardName?.message && <span className="text-red-600">{String(errors.identityCardName.message)}Hãy nhập tên cccd</span>}
                  <p className="mt-2">Số chứng minh nhân dân/số hộ chiếu</p>
                  <input  {...register('identityCardNumber', { required: true })} type="text" placeholder="xxxxxxxxxxx" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
                  {errors.identityCardNumber && <span className="text-red-600">{String(errors.identityCardNumber.message)}</span>}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-col gap-4 bg-white w-full p-5 rounded-xl  ">
        ...
        <div>
          ...
          <input {...register('TIN', { required: true })} type="text" placeholder="HãyHãy nhập 9 đến 15 ký tự" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
          {errors.TIN && (<p className="text-red-600">{String(errors.TIN.message)}</p>)}
        </div>
        <div>
        ...
        <FormInfo info={info[2]} >
          <FormInput{...register('taxRegistrationDocument', { required: true })} />
        </FormInfo>
      </div>
      <div className="flex flex-col gap-4 bg-white w-full p-5 rounded-xl  ">
        ...
        <div>
          ...
          <input {...register('bankAccountName', { required: true })} type="text" placeholder="Vui lòng điền tiến Việt, Không dấu" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
          {errors.bankAccountName?.message && <p className="text-red-500">{String(errors.bankAccountName.message)}</p>}
        </div>
        <div>
          <p className="mt-2">Số tài khoản</p>
          <input {...register('bankAccountNumber', { required: true })} type="text" placeholder="VD: 0123456789" className="w-[500px] p-2  border-2 border-gray-400 rounded-xl" />
          {errors.bankAccountNumber && <p className="text-red-500">{(String(errors.bankAccountNumber.message))}Nhập số tài khoản </p>}
        </div>
              ...
              {...register("bankName", {
                ...
                onChange: (e) => {
                  ...
                  if (selectedOption) {
                    ...
                    setValue("bankCode", selectedOption.code); // Gán code
                  }
                },
              })}
              defaultValue=""
        ...
        <div className="relative">
          ...
          <input type="text"
            ...
            {...register("bankBranch")}
          />
          {listSearchChiNhanh.length > 0 && (<div className="w-[500px] bg-white flex flex-col absolute top-[-29%] left-0 border-2 border-black rounded-lg   " >
            {listSearchChiNhanh.length > 0 ? (
              listSearchChiNhanh.map((bank, index) => (
                <p key={index} className="hover:bg-gray-200 p-2">{bank.name}</p>
              ))
            ) : (<p></p>)}
          </div>)}
        </div>
